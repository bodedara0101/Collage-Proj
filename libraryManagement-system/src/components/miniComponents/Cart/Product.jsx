...
import { DelateProduct } from "../../../store/slices/Slice";
import { PlusPrice } from "../../../store/slices/Slice";
import { Link } from 'react-router-dom';

const Product = ({ name, image, price }) => {
  const [qty, sqty] = useState(1);
  const [productPrice, sProductPrice] = useState(price);
    ...
    <>
      <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 md:p-6">
        <div className="space-y-4 md:flex md:items-center md:justify-between md:gap-6 md:space-y-0">
          ...
          <span htmlFor="counter-input" className="sr-only">
            Choose quantity:
          </span>
              ...
              <button
                ...
                onClick={() => {
                  qty == 0
                    ...
                    : qty > 0
                    ? (sqty(qty - 1),
                      sProductPrice(productPrice - price),
                      dis(PlusPrice({ price: -price })))
                    : sqty(0);
                }}
              >
                <svg
          ...
          <div className="w-full min-w-0 flex-1 space-y-4 md:order-2 md:max-w-md">
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                onClick={() => dis(DelateProduct(name))}
              >
                <svg
                  className="me-1.5 h-5 w-5"
                  aria-hidden="true"
